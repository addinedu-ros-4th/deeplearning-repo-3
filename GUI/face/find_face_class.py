...
class FaceDetector:
    ...
    def load_models(
        ...
        self.gender_net = cv2.dnn.readNetFromCaffe(
            gender_net_prototxt, gender_net_caffemodel
        )
    ...
    def detect_faces_and_info(self, frame):
        ...
        for x, y, w, h in results:
            ...
            face_distances = face_recognition.face_distance(
                self.known_face_encodings, face_encoded
            )
            ...
            if match_percentage >= 60:
                ...
                if match_percentage >= 80:
                    print(
                        f"⚠️ {name} 와(과) {match_percentage:.0f}% 유사 [ {age_gender_info}대로 추정 ]"
                    )
            else:
                name = "Unknown"
                text = name
            ...
            cv2.putText(
                frame, text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2
            )
            cv2.putText(
                frame,
                f"{age_gender_info}",
